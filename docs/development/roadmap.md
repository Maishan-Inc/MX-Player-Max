...
在阶段 5 的渲染器和滤镜接口之上，实现 AI 帧变换层。**与 WASM 解码器无关**——AI 消费已解码的 `VideoFrame`，与解码后端正交。

采用拉取式帧源：呈现循环按音频时钟时刻从后处理链拉取帧，而非推送式滤镜。管线顺序为 色彩转换 → 插帧 → 超分 → 滤镜 → 渲染 → 字幕覆盖。

分期交付：契约层（类型、能力探测、策略打分、passthrough 骨架）→ 超分 WGSL → 运行时 governor → 插帧 WGSL → 跨浏览器加固。**模型许可证与专利审查必须在契约层阶段完成**，不得推迟。

需要把阶段 8 的 manifest schema、加载器和哈希校验提前到本阶段（发布渠道仍留在阶段 8）。
