...
> 本节的视觉边界已由 `ADR-0006` 取代：演示站视觉现在对齐 MX-Player-Pro 的落地页结构，播放器 chrome
> 对齐 MXAnime-CMS 内置的 MX-Player。仍然成立的部分是：不复制 Pro 的实现文件、不复制其品牌资源。
> 下面列出的交互决策继续有效。

外观参考 `MX-Player-Pro` 的**交互模式**，不复制其实现文件。
