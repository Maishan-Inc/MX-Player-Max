...
import type { PlayerRenderMode, RenderModeAdapter, TheaterModeAdapter } from '@mx-player-max/ui'
...
import { mediaSummaryParts } from './media-summary'
import {
...
/**
 * The launcher selector and the player's settings panel are two views of one render path, so
 * both write through this adapter and neither can drift from the other.
 */
class DemoRenderMode implements RenderModeAdapter {
  #mode: PlayerRenderMode = 'native'
  readonly #listeners = new Set<(mode: PlayerRenderMode) => void>()

  getState(): PlayerRenderMode { return this.#mode }
  setState(mode: PlayerRenderMode): void {
...
export default function App() {
  ...
  const aiModelBaseUrl = resolveAiModelBaseUrl(import.meta.env.MODE, window.location.href)
  const rootRef = useRef<HTMLDivElement>(null)
  const playerRef = useRef<MXPlayerComponentHandle>(null)
  const languageRef = useRef<HTMLDivElement>(null)
  const theaterMode = useMemo(() => new DemoTheaterMode(), [])
  const renderModeAdapter = useMemo(() => new DemoRenderMode(), [])
  ...
  const [renderMode, setRenderMode] = useState<PlayerRenderMode>(() => renderModeAdapter.getState())
  ...
  const [mediaParts, setMediaParts] = useState<readonly string[]>([])
  ...
  // A new render path is a new session, so the diagnostics panels reset with it.
  useLayoutEffect(() => renderModeAdapter.subscribe((mode) => {
    setRenderMode(mode)
  ...
  /**
   * The summary under the player names the file that is actually decoding: resolution, frame rate,
   * codecs, channels and the selected backend. Only `ready` and `load` snapshots can change it, so
   * the string is compared before committing and the landing page is not re-rendered on every
   * time update.
   */
  useEffect(() => {
    const player = diagnosticPlayer
    if (!player) return
    const read = (): void => {
      let parts: readonly string[] = []
      try {
        parts = mediaSummaryParts({
          media: player.media,
    ...
    const offPlayback = player.on('playbackchange', ({ reason }) => { if (reason === 'load' || reason === 'capabilities') read() })
    const offError = player.on('error', read)
    return (): void => { offReady(); offPlayback(); offError() }
  }, [diagnosticPlayer, diagnosticRevision, copy.player.mediaPending])
  ...
  const playerOptions = useMemo(() => {
    const { intent, renderer } = renderModePlayerOptions(renderMode)
    return {
      source,
      intent,
      native: { preload: 'metadata' as const, crossOrigin: 'anonymous' as const },
      subtitles: { enabled: true },
      ...(renderer === null ? {} : { customVideo: { renderer } }),
      ...(aiModelBaseUrl === undefined ? {} : { aiModelBaseUrl }),
    }
  }, [source, renderMode, aiModelBaseUrl])

  const uiOptions = useMemo(() => ({
    theme,
    locale,
    theaterMode,
    renderMode: renderModeAdapter,
  ...
  }), [theme, locale, theaterMode, renderModeAdapter, mediaUrl, copy.documentTitle])

  const mediaReadout = mediaParts.join('  ·  ')
  ...
  const attachSubtitle = async (event: ChangeEvent<HTMLInputElement>): Promise<void> => {
    ...
    if (!file) return
    try {
      const track = await playerRef.current?.player?.addSubtitleTrack({ kind: 'file', file }, { name: file.name })
      ...
      <main className="home-main">
        <section className="player-launcher" aria-labelledby="player-heading" data-demo-reveal>
          ...
          <div className="source-summary">
            ...
            <span className="media-readout" aria-label={copy.player.mediaLabel} title={mediaReadout || copy.player.mediaPending}>
              {mediaParts.length > 0 ? mediaParts.map((part) => <em key={part}>{part}</em>) : <em>{copy.player.mediaPending}</em>}
            </span>
          </div>
          ...
          <div className="launcher-actions">
            ...
            <div className="mode-field">
              ...
              <select
                id="playback-intent"
                value={intentValueFromRenderMode(renderMode)}
                onChange={(event) => renderModeAdapter.setState(renderModeFromIntentValue(event.target.value))}
              >
                {RENDER_MODE_INTENT_VALUES.map((value) => (
                  <option key={value} value={value}>{copy.player.renderModes[value]}</option>
                ))}
              </select>
            </div>
          </div>
